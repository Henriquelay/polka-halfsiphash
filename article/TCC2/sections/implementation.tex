...
\pIV is a language used to program the data plane of network devices\cite{p4}. Behavioral Model 2 (\bmv) is a software switch that supports the \pIV language \cite{bmv2}\mycomment{descobrir pq o `2` não renderiza no cite}. Mininet is a network emulation tool that allows the creation of virtual networks with a controlled environment\cite{mininet}. Mininet-wifi\footnote{\url{github.com/intrig-unicamp/mininet-wifi}} is a Mininet fork used in this work due to its compatibility with \bmv. Despite the name, no Wi-Fi emulating feature was used.
...
%     \centering
    ...
    \begin{tikzpicture}
        ...
        \draw[->, very thick] (nhop.east) ..controls ++(0,-1) and ++(0,-0.5) .. (deparse.195);
